(function ($) {
    ...
    $(window).on('scroll', function () {
        if ($(window).scrollTop() < 50) {
            $('.ar-navbar').removeClass('active')
        } else {
            $('.ar-navbar').addClass('active')
        }
        $('.ar-navbar').toggleClass('hide', $(window).scrollTop() > Zero);
    ...
    $('').on("click", function () {
        $('services-more').addClass("active")
    })

    $('.open-services-more, .open-media-more, .open-see-more').on('click', function () {
        $(this).fadeOut();
        $('.services-more').slideDown();
        $('.media-more').slideDown();
        $('.see-more').slideDown();
    });

    $('a.portfolio__area-two-item').magnificPopup({
        type: 'image',
        ...
        image: {
            ...
            titleSrc: function (item) {
                return `<div class="caption">${item.el.attr("title")}</div>` + `<div class="subcaption">${item.el.attr("data-subtitle")}</div>`;
            }
        }
    });
